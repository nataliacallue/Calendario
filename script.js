...
// Inicializa selectores de mes y año
...
function renderCalendar() {
  ...
  for(let day=1; day<=lastDay.getDate(); day++){
    ...
    // Días con examen y/o clases extra
    if(event?.alumnos && event?.clases){
      div.classList.add("split");
      div.innerHTML += `<div>${event.clases} ext</div><div>${event.alumnos} Al.s</div>`;
    } else if(event?.alumnos){
      div.classList.add("day-red");
      div.innerHTML += `<div>${event.alumnos} Al.s</div>`;
    } else if(event?.clases){
